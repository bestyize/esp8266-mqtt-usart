...
#include "osapi.h"
#include "os_type.h"
#include "../user/repost_interface.h"
...
int mystrstr(char* buf, char* will);
...
int mystrstr(char* buf, char* will){
	int i = 0, j = 0;
		while ((i<=strlen(buf)-1)&&j<=strlen(will)-1) {
			if (buf[i] == will[j]) {
				i++;
				j++;
			}
...
LOCAL void uart0_rx_intr_handler(void *para)
{
	int fifo_len;
	    ...
	    onUsartDataReceive(recieveData);
...
uart_recvTask(os_event_t *events)
{
    if(events->sig == 0){
        ...
        uint8 d_tmp = 0;
        uint8 idx=0;
        uint8 receiveData[fifo_len];

        for(idx=0;idx<fifo_len;idx++) {
            d_tmp = READ_PERI_REG(UART_FIFO(UART0)) & 0xFF;
            receiveData[idx]=d_tmp;//接收到的数据存储到数组
